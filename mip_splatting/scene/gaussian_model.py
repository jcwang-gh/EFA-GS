...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_point_cloud = fused_point_cloud + torch.randn_like(fused_point_cloud) * 1.5
    ...
    def densify_lff(self, grads, grad_threshold, grads_abs, grad_abs_threshold, cameras, N=1, scaling_multiplier_max=1.0, scaling_multiplier_min=1.0, training_percent_powered=0.0, splitting_ub=1.0, splitting_lb=0.5, diffscale=True):
        ...
        # enlarged_opacity_changes = torch.zeros_like(self._opacity[enlarged_mask], dtype=torch.float32, device=self._opacity.device)
        ...
        with torch.no_grad():
            ...
            # opacity_multiplier_coef = (1 - interval_coef) * scaling_multiplier_min + interval_coef * scaling_multiplier_max
            log_scaling_multiplier = torch.log(scaling_multiplier_coef) * training_percent_powered
            # opacity_multiplier = torch.exp(torch.log(opacity_multiplier_coef) * training_percent_powered)
            ...
            # enlarged_opacity_changes += (self.inverse_opacity_activation(torch.min(opacity_multiplier * self.get_opacity, torch.ones_like(self.get_opacity)*0.999)) - self._opacity)[enlarged_mask]
        ...
        # splitted_opacity_changes = torch.zeros_like(self._opacity[splitted_mask], dtype=torch.float32, device=self._opacity.device)
        with torch.no_grad():
            # if function scaling_inverse_activation is not logarithm anymore, then use the following code.
            # scaling_changes = self.scaling_inverse_activation(self.get_scaling / torch.sqrt(torch.tensor(scaling_multiplier, device = self._scaling.device))) - self._scaling
            if diffscale:
            ...
            # splitted_opacity_changes += (self.inverse_opacity_activation(torch.min(opacity_multiplier * self.get_opacity, torch.ones_like(self.get_opacity)*0.999)) - self._opacity)[splitted_mask]
    ...
    def densify_and_prune_lff(self, max_grad, min_opacity, extent, max_screen_size, cameras, N=1, scaling_multiplier_max=1.0, scaling_multiplier_min=1.0,training_percent_powered=0.0, splitting_ub=1.0, splitting_lb=0.5, islff=False, diffscale=True):
        ...
        if nan_mask.sum() > 0:
            print(f"During dapn, {nan_mask.sum()} points are NaN.")
