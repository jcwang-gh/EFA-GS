...
def calculate_training_percent_powered(iter, min_densify_iter, max_densify_iter, pow=1.0, lower_bound=0):
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, init_scaling_multiplier_max, last_scaling_multiplier_max, init_scaling_multiplier_min, pow, splitting_ub, splitting_lb, interval_times, diffscale):
    ...
    # calculate lower bound for function calculate_training_percent_powered
    percent_lb = log(last_scaling_multiplier_max) / log(init_scaling_multiplier_max) if init_scaling_multiplier_max != 1.0 else 1
    print(f"{last_scaling_multiplier_max=},{init_scaling_multiplier_min=}")
    if diffscale:
        print(f"diffscale is true")
    else:
        print("diffscale is false.")
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                gaussians.add_densification_stats_lff(viewspace_point_tensor, visibility_filter)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_scaling_multiplier_max", type=float, default=2.0)
    parser.add_argument("--last_scaling_multiplier_max", type=float, default=1.0)
    parser.add_argument("--init_scaling_multiplier_min", type=float, default=1.0)
